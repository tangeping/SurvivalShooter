...
namespace SyncFrame {
    ...
    public class Physics2DWorldManager : IPhysicsManager {
        ...
        public void RemoveBody(IBody iBody) {
            world.RemoveBody((SyncFrame.Physics2D.Body) iBody);
            world.ProcessRemovedBodies();
        }

        public void OnRemoveBody(Action<IBody> OnRemoveBody) {
            world.BodyRemoved += delegate (SyncFrame.Physics2D.Body body) {
                OnRemoveBody(body);
            };
        }

        private void OnRemovedRigidBody(SyncFrame.Physics2D.Body body) {
        ...
        private bool CollisionEnter(SyncFrame.Physics2D.Contact contact) {
        ...
        private void CollisionStay(SyncFrame.Physics2D.Contact contact) {
            if (contact.FixtureA.IsSensor || contact.FixtureB.IsSensor) {
                TriggerStay(contact);
                return;
            }
        ...
        private void CollisionExit(SyncFrame.Physics2D.Contact contact) {
            if (contact.FixtureA.IsSensor || contact.FixtureB.IsSensor) {
                TriggerExit(contact);
                return;
            }
        ...
        private void TriggerEnter(SyncFrame.Physics2D.Contact contact) {
            CollisionDetected(contact.FixtureA.Body, contact.FixtureB.Body, contact, "OnSyncedTriggerEnter");
        }

        private void TriggerStay(SyncFrame.Physics2D.Contact contact) {
            CollisionDetected(contact.FixtureA.Body, contact.FixtureB.Body, contact, "OnSyncedTriggerStay");
        }

        private void TriggerExit(SyncFrame.Physics2D.Contact contact) {
        ...
        public TSRaycastHit2D Raycast(TSVector2 origin, TSVector2 direction, FP distance) {
            ...
            Func<SyncFrame.Physics2D.Fixture, TSVector2, TSVector2, FP, FP> callback = delegate (SyncFrame.Physics2D.Fixture fixture, TSVector2 point, TSVector2 normal, FP fraction) {
